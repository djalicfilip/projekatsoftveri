...
namespace Repository
{
    public class GenericDbRepository : IDbRepository<IEntity>
    {
        ...
        public void AddWithItems(IEntity entity, List<IEntity> lista)
        {
            SqlCommand cmd = DbConnectionFactory.Instance.GetDbConnection().CreateCommand($"insert into {entity.TableName} output inserted.{entity.IdColumn} values ({entity.InsertedValues})");
            int id = (int)cmd.ExecuteScalar();
            foreach (IEntity stavka in lista)
            {
                SqlCommand cmd1 = DbConnectionFactory.Instance.GetDbConnection().CreateCommand($"insert into {stavka.TableName} values ({id}{stavka.InsertedValues})");
        ...
        {
            SqlCommand command = DbConnectionFactory.Instance.GetDbConnection().CreateCommand($"Delete from {entity.TableName} where {entity.IdColumn}={entity.ID} ");
        ...
        {
            List<IEntity> result;
            SqlCommand cmd = DbConnectionFactory.Instance.GetDbConnection().CreateCommand($"select {entity.ReturnedColumns} from {entity.TableNameJoin}");
        ...
        {
            List<IEntity> result;
            SqlCommand cmd = DbConnectionFactory.Instance.GetDbConnection().CreateCommand($"select {entity.ReturnedColumns} from {entity.TableNameJoin} where {condition}");
            SqlDataReader reader = cmd.ExecuteReader();
            result = entity.GetList(reader);
            reader.Close();
